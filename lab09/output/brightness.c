...
int main() {
    rgb_img *temp_image;
    read_in_img(&temp_image, "cannon.bin");
    ...
    for (int mult = 0; mult < 1; mult ++) {
        for (int i = 0; i < temp_image->width; i++) {
            for (int j = 0; j < temp_image->height; j++) {
                ...
                uint8_t r = get_pixel(temp_image, j, i, 0);
                uint8_t g = get_pixel(temp_image, j, i, 1);
                uint8_t b = get_pixel(temp_image, j, i, 2);
                r = (2 * r > 255) ? 255 : 2 * r;
                g = (2 * g > 255) ? 255 : 2 * g;
                b = (2 * b > 255) ? 255 : 2 * b;
                set_pixel(temp_image, j, i, r, g, b);
            }
        }
        sprintf(filename, "image_edited_%d.bin", mult);
        printf("%s\n", filename);
        write_img(temp_image, filename);
    }
